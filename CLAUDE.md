...
**Card creation is asymmetric.** Passive cards get created on first exposure (`transcript_clicked`, `status_marked_learning`). Active cards do NOT — they only come into existence when an event already includes a correct production outcome (`guided_counted`, `status_marked_known`, `free_chat_used_correctly`, `active_review_correct`). Consequence: a user who marks words "learning" and never opens guided chat has zero scheduled active practice. See `docs/WORKFLOW_AUDIT.md` Hole 11/16/0b.
...
**Auto-promotion is one-way.** Once `status='known'`, no event auto-demotes it. The user must manually mark it back. There is no spaced-forgetting check.
...
**SRS review is now real (since 2026-05-19, #0a complete).** Backend (#0a-1): `review_service.get_due_cards` carries `prompt_text` + `answer_text`. Glosses for word/phrase come from `llm_service.translate_item_gloss` (cached). Grammar rules use `title` / `short_explanation`. Frontend + production endpoint (#0a-2): `SRSReviewPage.tsx` consumes `prompt_text` / `answer_text`; passive direction is still self-graded; active direction requires typed German input → `POST /srs/review/{card_id}/produce` → `llm_service.evaluate_production` (or fast-path exact match) → standard SM-2 via `progression_service`. Self-grade is no longer exposed for active cards.
